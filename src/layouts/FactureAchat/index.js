...
const FactureAchatComponent = () => {
  ...
  useEffect(() => {
    Promise.all([fetchFactures(), fetchProduits(), fetchFournisseurs()]).then(() => {
      console.log("Initial data - Fournisseurs:", fournisseurs);
  ...
  const fetchFactures = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Échec de la récupération des factures d'achat.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateFacture = () => {
    ...
    setCurrentFacture({
      _id: "",
      numeroFacture: "", // Ignored by backend
      fournisseur: "",
      dateFacturation: new Date().toISOString().split("T")[0],
      dateEcheance: "",
      modePaiement: "",
  ...
  const handleEditFacture = async (facture) => {
    ...
    try {
      ...
      setCurrentFacture({
        ...
        fournisseur: res.data.fournisseur?._id || res.data.fournisseur || "",
      });
      setFactureItems(
  ...
  const generatePDF = (facture) => {
    try {
      ...
      if (!facture.fournisseur) {
        throw new Error("Fournisseur is missing");
      }
      ...
      doc.save(`facture_achat_${facture.numeroFacture || facture._id || "unknown"}.pdf`);
    } catch (err) {
  ...
  const filteredFactures = factures.filter((facture) => {
    ...
    const matchesSearch = query
      ? facture.numeroFacture.toLowerCase().includes(query) ||
        (facture.fournisseur?.nomRaisonSociale?.toLowerCase().includes(query) ?? false) ||
  ...
  const factureTableColumns = [
    ...
    { Header: "Fournisseur", accessor: "fournisseur", align: "center" },
  ...
  const factureTableRows = filteredFactures.map((facture) => ({
    ...
    fournisseur: facture.fournisseur?.nomRaisonSociale || "N/A",
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    ...
                    <TextField
                      ...
                      placeholder="Rechercher (numéro, fournisseur, etc.)"
                      variant="outlined"
                    />
                  </Grid>
                  <Grid item xs={12} sm={6}>
        ...
        <MDBox sx={modalStyle}>
          ...
          <form onSubmit={handleSubmitFacture}>
              ...
              <Grid item xs={6}>
                ...
                <Autocomplete
                  ...
                  getOptionLabel={(option) =>
                    `${option.numeroFournisseur} - ${option.nomRaisonSociale} (${
                      option.typeFournisseur || "N/A"
                    })`
                  }
                  value={fournisseurs.find((f) => f._id === currentFacture.fournisseur) || null}
                  onChange={(event, newValue) => {
                    setCurrentFacture((prev) => ({
                      ...prev,
                      fournisseur: newValue ? newValue._id : "",
                    }));
                  }}
                  renderInput={(params) => (
                    <TextField
            ...
            <MDBox mt={3}>
              ...
              {factureItems.length > 0 && (
                <MDTypography variant="h6" fontWeight="medium" mt={2} textAlign="right">
            ...
            <MDBox mt={3} display="flex" justifyContent="flex-end" gap={2}>
              ...
              <MDButton
                ...
                color="success"
                type="submit"
                disabled={factureItems.length === 0 || !currentFacture.fournisseur}
              >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                ...
                <Autocomplete
                  ...
                  value={
                    fournisseurs.find((f) => f._id === newProduit.fournisseurPrincipal) || null
                  }
                  onChange={(event, newValue) => {
                    setNewProduit((prev) => ({
                      ...prev,
                      fournisseurPrincipal: newValue ? newValue._id : "",
                    }));
                  }}
                  renderInput={(params) => (
                    <TextField
          ...
          <Grid container spacing={2}>
            {[
              { label: "Numéro Facture", value: currentFacture.numeroFacture },
              {
                label: "Fournisseur",
                value:
                  (typeof currentFacture.fournisseur === "object"
                    ? currentFacture.fournisseur?.nomRaisonSociale
                    : fournisseurs.find((f) => f._id === currentFacture.fournisseur)
...
export default FactureAchatComponent;
